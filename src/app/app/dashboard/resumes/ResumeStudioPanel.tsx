...
export function ResumeStudioPanel() {
  ...
  const createVacancyVersion = async (vacancy: VacancyLaunch) => {
    ...
    try {
      ...
      if (!payload?.provenance || !payload.analysis || !payload.matching) {
        throw new Error("Career Twin handoff is missing finalized content or provenance");
      }
    ...
    <section className="px-4 sm:px-6" aria-labelledby="resume-studio-title">
      <div className="rounded-2xl border border-stone-200 bg-[#f4f1e8] p-5 text-stone-950 shadow-sm dark:border-stone-800 dark:bg-stone-950 dark:text-stone-50 sm:p-7">
        ...
        <p className="mt-3 max-w-3xl text-sm leading-6 text-stone-600 dark:text-stone-300">
          Career Twin передаёт финальный текст под выбранную вакансию. Студия только формирует
          редактируемый документ, применяет шаблон и готовит экспорт — без анализа вакансии и
          переписывания содержания.
        </p>
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <div className="mt-1 text-xs text-stone-500 dark:text-stone-400">
                  Finalized by Career Twin · renderer-only handoff
                </div>
              </div>
              <div className="flex min-w-64 flex-col gap-2">
                ...
                <Button
                  ...
                  disabled={tailoring || sessionState !== "ready"}
                  className="bg-emerald-900 text-white hover:bg-emerald-800"
                >
                  {tailoring
                    ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Формирую документ…</>
                    : <><Sparkles className="mr-2 h-4 w-4" />Сформировать CV</>}
                </Button>
              </div>
            </div>
          </div>
